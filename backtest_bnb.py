...
class BacktestEngine:
    ...
    def open_position(self, side, entry_price, entry_time, tp_price, sl_price, strategy_config):
        """开仓（使用总仓位的10%作为保证金，5倍杠杆）"""
        if self.position:
            return  # 已有持仓，不重复开仓
        
        # 计算开仓数量（使用当前余额的10%作为保证金，5倍杠杆）
        leverage = 5
        margin = self.balance * 0.1  # 保证金（余额的10%）
        ...
        print(f"  保证金: {margin:.2f} USDT (余额的10%)")
...
def main():
    ...
    print("\n" + "="*60)
    print("BNB 回测 - Pinbar多周期共振策略 + 1H移动止损")
    print("="*60)
    print("策略说明:")
